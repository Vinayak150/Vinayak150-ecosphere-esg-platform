...
def test_dashboard_social_and_governance_unavailable(db_client, admin_tokens) -> None:
    response = db_client.get(
        "/api/v1/dashboard",
        headers={"Authorization": f"Bearer {admin_tokens['access_token']}"},
    )
    ...
    assert response.status_code == 200
    data = response.json()["data"]
    assert data["socialScore"]["available"] is False
    assert data["socialScore"]["score"] is None
    assert data["governanceScore"]["available"] is False
    assert data["governanceScore"]["score"] is None
