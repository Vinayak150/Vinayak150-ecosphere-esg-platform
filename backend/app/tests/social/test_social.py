...
def test_csr_activity_crud(db_client, admin_tokens, csr_activity_payload) -> None:
    ...
    update_response = db_client.put(
        ...
        headers={"Authorization": f"Bearer {admin_tokens['access_token']}"},
        json={"description": "Updated CSR description"},
    )
...
def test_participation_and_approval_flow(
    ...
    assert approved["completion_date"] is not None
...
def test_duplicate_participation_rejected(
    db_client, admin_tokens, created_csr_activity
) -> None:
    payload = {
        "csr_activity_id": created_csr_activity["id"],
        "proof_file": "https://storage.example.com/proof1.jpg",
    }
...
def test_social_analytics(db_client, admin_tokens, created_csr_activity) -> None:
    ...
    assert "department_participation" in analytics
    assert "monthly_csr_trend" in analytics
    assert "top_participating_departments" in analytics
    assert Decimal(analytics["social_score"]) >= 0


def test_dashboard_includes_social_score(db_client, admin_tokens) -> None:
    response = db_client.get(
        "/api/v1/dashboard",
        headers={"Authorization": f"Bearer {admin_tokens['access_token']}"},
    )
